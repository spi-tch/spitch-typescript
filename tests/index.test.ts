...
import { APIPromise } from 'spitch/core/api-promise';
...
describe('instantiate client', () => {
  ...
  describe('logging', () => {
    ...
    beforeEach(() => {
      process.env = { ...env };
      process.env['SPITCH_LOG'] = undefined;
    });
    ...
    const forceAPIResponseForClient = async (client: Spitch) => {
    ...
    test('debug logs when log level is debug', async () => {
      ...
      const client = new Spitch({ logger: logger, logLevel: 'debug', apiKey: 'My API Key' });
    ...
    test('debug logs are skipped when log level is info', async () => {
      ...
      const client = new Spitch({ logger: logger, logLevel: 'info', apiKey: 'My API Key' });
    ...
    test('debug logs happen with debug env var', async () => {
      ...
      process.env['SPITCH_LOG'] = 'debug';
      const client = new Spitch({ logger: logger, apiKey: 'My API Key' });
    ...
    test('warn when env var level is invalid', async () => {
      ...
      process.env['SPITCH_LOG'] = 'not a log level';
      const client = new Spitch({ logger: logger, apiKey: 'My API Key' });
      expect(client.logLevel).toBe('warn');
      expect(warnMock).toHaveBeenCalledWith(
        'process.env[\'SPITCH_LOG\'] was set to "not a log level", expected one of ["off","error","warn","info","debug"]',
      );
    });
    ...
    test('client log level overrides env var', async () => {
      ...
      process.env['SPITCH_LOG'] = 'debug';
      const client = new Spitch({ logger: logger, logLevel: 'off', apiKey: 'My API Key' });
  ...
  describe('withOptions', () => {
    test('creates a new client with overridden options', async () => {
      const client = new Spitch({ baseURL: 'http://localhost:5000/', maxRetries: 3, apiKey: 'My API Key' });
    ...
    test('respects runtime property changes when creating new client', () => {
      const client = new Spitch({ baseURL: 'http://localhost:5000/', timeout: 1000, apiKey: 'My API Key' });
