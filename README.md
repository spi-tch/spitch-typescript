...
```ts
async function fetchAllFileMetas(params) {
  const allFileMetas = [];
  // Automatically fetches more pages as needed.
  for await (const fileMeta of client.files.list({ limit: 50 })) {
    allFileMetas.push(fileMeta);
  }
  return allFileMetas;
}
```
...
for (const fileMeta of page.items) {
